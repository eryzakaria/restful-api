...
func SetupTestDB() *sql.DB {
	db, err := sql.Open("mysql", "root:1234567890@tcp(localhost:3306)/restful_api_go_test")
...
func TestCreateCategorySuccess(t *testing.T) {
	...
	request.Header.Add("X-API-Key", "NARUTO1234")
...
func TestGetCategoryFailde(t *testing.T) {
	db := SetupTestDB()
	truncateCategory(db)
	router := SetupRouter(db)
